---
title: "Seurat - Guided Clustering Tutorial- Inserting A375 Parental Data"
output:
  html_document:
    theme: united
    df_print: kable
...
For this analysis, we will be analyzing the 3 datasets of A375 melanoma data from the Hangauer laboratory using 10X Genomics sequencing. Dataset 1 is A375 parental cells that did not recieve any drug treatment. Dataset 2 is A375 persister cells treated with 0.25uM Dabrafenib (BRAF inhibitor) + 25 nM Trametinib (MEK inhibitor). Dataset 3 is A375 Drug-tolerant Expanded Persister (DTEP) cells in 0.25 uM Dabrafenib + 25 nM Trametinib.
...
#apar.data <- Read10X(data.dir = "~/Desktop/apar_output_files/filtered_feature_bc_matrix")
# Initialize the Seurat object with the raw (non-normalized data).
#apar <- CreateSeuratObject(counts = apar.data, project = "A375_parental", min.cells = 3, min.features = 200)
#apar
```

```{r}
# Load the A375 persister dataset
apers.data <- Read10X(data.dir = "~/Desktop/apers_output_files/filtered_feature_bc_matrix")
# Initialize the Seurat object with the raw (non-normalized data).
apers <- CreateSeuratObject(counts = apers.data, project = "A375_persisters", min.cells = 3, min.features = 200)
apers
```

```{r}
# Load the A375 DTEP dataset
adtep.data <- Read10X(data.dir = "~/Desktop/adtep_output_files/filtered_feature_bc_matrix")
# Initialize the Seurat object with the raw (non-normalized data).
adtep <- CreateSeuratObject(counts = adtep.data, project = "A375_dtep", min.cells = 3, min.features = 200)
adtep
```
### I am now going to merge some Seurat objects together for analysis. 
...
```{r}
pers.vs.dtep <- merge(apers, y = adtep, add.cell.ids = c("pers", "dtep"), project = "A375_pers_vs_dtep")
pers.vs.dtep
```
```{r}
head(colnames(pers.vs.dtep))
```
```{r}
tail(colnames(pers.vs.dtep))
```
```{r}
table(pers.vs.dtep$orig.ident)
```
...
pers.vs.dtep[["percent.mt"]] <- PercentageFeatureSet(object = pers.vs.dtep, pattern = "^MT-")
```
...
head(x = pers.vs.dtep@meta.data, 5)
```
</details>
\  
...
* We filter cells that have unique feature counts over 2,500 or less than 200. I am not doing any filtering yet.
* We filter cells that have >5% mitochondrial counts. No filtering yet
    
```{r qc2, fig.height=7, fig.width=13}

#Visualize QC metrics as a violin plot
VlnPlot(object = pers.vs.dtep, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)

# FeatureScatter is typically used to visualize feature-feature relationships, but can be used for anything calculated by the object, i.e. columns in object metadata, PC scores etc.

plot1 <- FeatureScatter(object = pers.vs.dtep, feature1 = "nCount_RNA", feature2 = "percent.mt") 
plot2 <- FeatureScatter(object = pers.vs.dtep, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") 
CombinePlots(plots = list(plot1,plot2))

# not going to filter for now. But will come back to this step for 2nd pass through.
...
```{r normalize}
pers.vs.dtep <- NormalizeData(object = pers.vs.dtep, normalization.method = "LogNormalize", scale.factor = 1e4)
```
...
pers.vs.dtep <- FindVariableFeatures(object = pers.vs.dtep,selection.method = 'vst', nfeatures = 2000)

# Identify the 20 most highly variable genes
top20 <- head(x = VariableFeatures(object = pers.vs.dtep), 20)
top20

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(object = pers.vs.dtep)
...
all.genes <- rownames(x = pers.vs.dtep)
pers.vs.dtep <- ScaleData(object = pers.vs.dtep, features = all.genes)
```
<details>
...
# do not do this unless you only want 2,000 genes to be used. pbmc <- ScaleData(object = pbmc)
```
...
```{r pca,results='hide'}
pers.vs.dtep <- RunPCA(object = pers.vs.dtep, features = VariableFeatures(object = pers.vs.dtep))
```
...
print(x = pers.vs.dtep[['pca']], dims = 1:5, nfeatures = 5)
VizDimLoadings(object = pers.vs.dtep, dims = 1:2, reduction = 'pca')
DimPlot(object = pers.vs.dtep, reduction = 'pca')
```
...
```{r single-heatmap}
?DimHeatmap
DimHeatmap(object = pers.vs.dtep, dims = 1, cells = 500, balanced = TRUE)
```

```{r multi-heatmap, fig.height=12, fig.width=9}
DimHeatmap(object = pers.vs.dtep, dims = 1:15, cells = 500, balanced = TRUE)
```
...
pers.vs.dtep <- FindNeighbors(object = pers.vs.dtep, dims = 1:15)
pers.vs.dtep <- FindClusters(object = pers.vs.dtep, resolution = 0.5)

# Look at cluster IDs of the first 5 cells
head(x = Idents(object = pers.vs.dtep), 5)
```
...
pers.vs.dtep <- RunUMAP(object = pers.vs.dtep, dims = 1:15)
```

```{r tsneplot, fig.height=5, fig.width=7}
# note that you can set `label = TRUE` or use the LabelClusters function to help label individual clusters
DimPlot(object = pers.vs.dtep, reduction = 'umap', label = TRUE, group.by = "orig.ident")
```

```{r tsneplot, fig.height=5, fig.width=7}
DimPlot(object = pers.vs.dtep, reduction = 'umap', label = TRUE)
```

You can save the object at this point so that it can easily be loaded back in without having to rerun the computationally intensive steps performed above, or easily shared with collaborators.

```{r saveobject, eval=FALSE}
saveRDS(all.combined, file = "~/Desktop/A375_scRNAseq_analysis.rds")
dat <- readRDS("ACS.rds")
```
...
cluster.persister.markers <- FindMarkers(object = pers.vs.dtep, ident.1 = "A375_persisters", group.by = "orig.ident", min.pct = 0.25)
head(x = cluster.persister.markers, n = 100)
```

```{r markers1, fig.height=8, fig.width=15}
# find all markers distinguishing cluster 5 from clusters 0 and 3
cluster5.markers <- FindMarkers(object = all.combined, ident.1 = 5, ident.2 = c(0, 3), min.pct = 0.25)
head(x = cluster5.markers, n = 5)
# find markers for every cluster compared to all remaining cells, report only the positive ones
pers.dtep.markers <- FindAllMarkers(object = pers.vs.dtep, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
pers.dtep.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_logFC)
```
...
cluster1.markers <- FindMarkers(object = pers.vs.dtep, ident.1 = 0, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
```

We include several tools for visualizing marker expression. `VlnPlot` (shows expression probability distributions across clusters), and `FeaturePlot` (visualizes feature expression on a tSNE or PCA plot) are our most commonly used visualizations. We also suggest exploring `RidgePlot`, `CellScatter`, and `DotPlot` as additional methods to view your dataset.
```{r markerplots, fig.height=8, fig.width=15}
VlnPlot(object = pers.vs.dtep, features = c("BSL2L11", "BAX"))
```

```{r markerplots, fig.height=8, fig.width=15}
VlnPlot(object = pers.vs.dtep, features = c("GPX4", "DFFB"), group.by = "orig.ident")
```

```{r markerplots, fig.height=8, fig.width=15}
VlnPlot(object = pers.vs.dtep, features = c("MITF", "CD24"), group.by = "orig.ident")
```

```{r markerplots, fig.height=8, fig.width=15}
# you can plot raw counts as well
#VlnPlot(object = pbmc, features = c("NKG7", "PF4"), slot = 'counts', log = TRUE)
FeaturePlot(object = pers.vs.dtep, features = c("MKI67", "MITF", "GPX4", "DFFB"))
```
```{r markerplots, fig.height=8, fig.width=15}
#Pro-apoptotic genes Matt and Mike are interested in BIM/BAX/BAK
FeaturePlot(object = pers.vs.dtep, features = c("BCL2L11", "BAX", "BAK1"))
```

```{r markerplots, fig.height=8, fig.width=15}
#Anti-apoptotic genes Matt and Mike are interested in BCL2/BCLXL/MCL1/BFL-1
FeaturePlot(object = pers.vs.dtep, features = c("BCL2", "BCL2L1", "MCL1", "BCL2A1"))
```

`DoHeatmap` generates an expression heatmap for given cells and features. In this case, we are plotting the top 20 markers (or all markers if less than 20) for each cluster.

```{r clusterHeatmap, fig.height=8, fig.width=15}
pers.dtep.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC) -> top10
DoHeatmap(object = pers.vs.dtep, features = top10$gene)
```
```{r clusterHeatmap, fig.height=8, fig.width=15}
pers.dtep.markers %>% group_by("orig.ident") %>% top_n(n = 100, wt = avg_logFC) -> top100
DoHeatmap(object = pers.vs.dtep, features = top100$gene, group.by = "orig.ident")
```
```{r}
head(top100, n=30)
```
...
library(ggplot2)
plot <- DimPlot(object = all.combined, reduction = "umap", label = TRUE, label.size = 4.5) + xlab("UMAP 1") + ylab("UMAP 2") + 
  theme(axis.title = element_text(size = 18), legend.text = element_text(size = 16)) + 
