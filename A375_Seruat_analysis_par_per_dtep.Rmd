...
```{r}
all.combined <- merge(apar, y = c(apers, adtep), add.cell.ids = c("par", "pers", "dtep"), project = "A375_all_merged")
all.combined
```
```{r}
...
head(x = all.combined@meta.data, 5)
```
</details>
\  
...
VlnPlot(object = all.combined, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)

# FeatureScatter is typically used to visualize feature-feature relationships, but can be used for anything calculated by the object, i.e. columns in object metadata, PC scores etc.

plot1 <- FeatureScatter(object = all.combined, feature1 = "nCount_RNA", feature2 = "percent.mt") 
plot2 <- FeatureScatter(object = all.combined, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") 
...
```{r normalize}
all.combined <- NormalizeData(object = all.combined, normalization.method = "LogNormalize", scale.factor = 1e4)
```
...
all.combined <- FindVariableFeatures(object = all.combined,selection.method = 'vst', nfeatures = 2000)

# Identify the 20 most highly variable genes
top20 <- head(x = VariableFeatures(object = all.combined), 20)
top20

# plot variable features with and without labels
plot1 <- VariableFeaturePlot(object = all.combined)
...
all.genes <- rownames(x = all.combined)
all.combined <- ScaleData(object = all.combined, features = all.genes)
```
<details>
...
```{r pca,results='hide'}
all.combined <- RunPCA(object = all.combined, features = VariableFeatures(object = all.combined))
```
...
print(x = all.combined[['pca']], dims = 1:5, nfeatures = 5)
VizDimLoadings(object = all.combined, dims = 1:2, reduction = 'pca')
DimPlot(object = all.combined, reduction = 'pca')
```
...
```{r single-heatmap}
?DimHeatmap
DimHeatmap(object = all.combined, dims = 1, cells = 500, balanced = TRUE)
```

```{r multi-heatmap, fig.height=12, fig.width=9}
DimHeatmap(object = all.combined, dims = 1:15, cells = 500, balanced = TRUE)
```
...
all.combined <- FindNeighbors(object = all.combined, dims = 1:10)
all.combined <- FindClusters(object = all.combined, resolution = 0.5)

# Look at cluster IDs of the first 5 cells
head(x = Idents(object = all.combined), 5)
```
...
all.combined <- RunUMAP(object = all.combined, dims = 1:10)
```

```{r tsneplot, fig.height=5, fig.width=7}
# note that you can set `label = TRUE` or use the LabelClusters function to help label individual clusters
DimPlot(object = all.combined, reduction = 'umap', label = TRUE, group.by = "orig.ident")
```

```{r tsneplot, fig.height=5, fig.width=7}
DimPlot(object = all.combined, reduction = 'umap', label = TRUE)
```
...
cluster.persister.markers <- FindMarkers(object = all.combined, ident.1 = "A375_persisters", group.by = "orig.ident", min.pct = 0.25)
...
all.combined.markers <- FindAllMarkers(object = all.combined, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
all.combined.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_logFC)
```
...
cluster1.markers <- FindMarkers(object = all.combined, ident.1 = 0, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
```

We include several tools for visualizing marker expression. `VlnPlot` (shows expression probability distributions across clusters), and `FeaturePlot` (visualizes feature expression on a tSNE or PCA plot) are our most commonly used visualizations. We also suggest exploring `RidgePlot`, `CellScatter`, and `DotPlot` as additional methods to view your dataset.
```{r markerplots, fig.height=8, fig.width=15}
VlnPlot(object = all.combined, features = c("GPX4", "DFFB"))
```

```{r markerplots, fig.height=8, fig.width=15}
VlnPlot(object = all.combined, features = c("GPX4", "DFFB"), group.by = "orig.ident")
```

```{r markerplots, fig.height=8, fig.width=15}
VlnPlot(object = all.combined, features = c("MITF", "CD24"), group.by = "orig.ident")
```

```{r markerplots, fig.height=8, fig.width=15}
# you can plot raw counts as well
#VlnPlot(object = pbmc, features = c("NKG7", "PF4"), slot = 'counts', log = TRUE)
FeaturePlot(object = all.combined, features = c("MKI67", "MITF", "GPX4", "DFFB"))
```
```{r markerplots, fig.height=8, fig.width=15}
#Pro-apoptotic genes Matt and Mike are interested in BIM/BAX/BAK
FeaturePlot(object = all.combined, features = c("BCL2L11", "BAX", "BAK1"))
```

```{r markerplots, fig.height=8, fig.width=15}
#Anti-apoptotic genes Matt and Mike are interested in BCL2/BCLXL/MCL1/BFL-1
FeaturePlot(object = all.combined, features = c("BCL2", "BCL2L1", "MCL1", "BCL2A1"))
```

`DoHeatmap` generates an expression heatmap for given cells and features. In this case, we are plotting the top 20 markers (or all markers if less than 20) for each cluster.

```{r clusterHeatmap, fig.height=8, fig.width=15}
all.combined.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC) -> top10
DoHeatmap(object = all.combined, features = top10$gene) + NoLegend()
```
```{r clusterHeatmap, fig.height=8, fig.width=15}
all.combined.markers %>% group_by("orig.ident") %>% top_n(n = 100, wt = avg_logFC) -> top100
DoHeatmap(object = all.combined, features = top100$gene, group.by = "orig.ident")
```
```{r}
head(top100, n=30)
```
...
```{r}
```{r save.rds}
#saveRDS(all.combined, file = "~/Desktop/A375_scRNAseq_final.rds")
```